...
2. **필드 단위 암호화 — `SocialAccount.refreshToken`** — OAuth 리프레시 토큰은 민감 자격증명이고 값으로 조회되지 않으므로 AES-256-GCM(랜덤 IV) + JPA `AttributeConverter`(`AesGcmStringConverter`)로 암호화한다. 암호화 키는 `widyu.encryption.aes-key`로 주입한다(운영·개발은 환경변수 필수). 기존 평문 데이터는 암호화 접두사 유무로 구분해 그대로 읽고, 쓰기 시 암호화해 점진적으로 전환한다. 조회 쿼리에 영향이 없다. (본 PR에서 구현)
...
- refreshToken은 DB 접근 수준에서도 원문이 노출되지 않는다(본 PR에서 구현, 상세는 LLD-0028).
- 기존 평문 refreshToken은 읽기 호환으로 유지되다 재저장 시 점진 암호화된다. 즉시 전량 전환이 필요하면 백필 배치를 수행한다(추적 이슈 #586, 상세 LLD-0028).
- 식별자 필드는 당분간 평문으로 DB에 저장되며, 저장소 암호화에 의존한다. 법적 요구 변화 시 본 ADR을 갱신하고 결정적 암호화를 재검토한다.
- 키 관리(주입·교체)는 운영 절차로 관리하며, 향후 KMS 도입을 검토한다. 배포 환경에 `WIDYU_ENCRYPTION_AES_KEY`(Base64 32바이트)를 설정해야 한다.
