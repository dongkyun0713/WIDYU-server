...
| 항목 | 값 |
| --- | --- |
| 상태 | Superseded by ADR-0034 |
| 날짜 | 2026-09-16 |
| 관련 | LLD-0031, #617, `apiDocs/위듀_데이터정책서_v0.md` 1.5.1~1.5.3 |
| 대체 | [ADR-0034](ADR-0034-study-participation-record.md)가 대체한다. 클라이언트 발급 식별자·필수 보존 날짜·자동 가명화/파기와 "기간 변경 이력은 감사 로그로 충분"이라는 결정을 모두 되돌렸다 |

> **2026-09-21 폐기.** [ADR-0034](ADR-0034-study-participation-record.md)가 대체한다. 실증 운영에서 IRB 승인 전에 참가자를 먼저 등록해야 해 보존 날짜를 필수로 둘 수 없었고, 자동 가명화·파기는 채택하지 않았으며, 감사 로그 문자열로는 과거 보관 계획과 철회 범위를 복원할 수 없어 불변 snapshot 이력으로 바꿨다. 아래 내용은 이력용이다.
