...
- 기존 저장분은 접두사가 없어 그대로 읽히고, 재저장(재로그인·토큰 갱신) 시점부터 암호화되어 점진 전환된다. 강제 백필(전체 재암호화)은 선택 사항이며 필요 시 별도 배치로 수행한다(추적 이슈 #586).
- 본 저장소는 Flyway/Liquibase 등 추적 마이그레이션 체계를 쓰지 않고 Hibernate `ddl-auto: update`로 스키마를 관리한다. `refresh_token` 길이 1000→2048 확대는 ddl-auto의 컬럼 확장으로 반영된다. nullable 여부는 기존과 동일(제약 없음)하게 유지한다.

## 9. 미결정 사항 (Open Questions)

- 기존 평문 refreshToken의 강제 백필 시점·방식(즉시 배치 vs 자연 전환) — 추적 이슈 #586.
