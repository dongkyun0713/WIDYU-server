...
- `devices[].role ∈ watch | phone | external_ecg | operator_marker`, `wearSite` 문자열(값 집합 S1 미정). 같은 `deviceId`가 **다른 열린 회차**에 배정돼 있으면 409(기기는 한 번에 한 참가자).
- 회원당 열린 회차는 1개(409).
...
인덱스: UK `run_id`; `idx_collection_run_member_status (member_id, status)`.

### `run_device_assignment`

`id`, `assignment_id` VARCHAR(40) UK(`asg-` + UUID 32 hex), `run_id` FK collection_run(id), `device_id` VARCHAR(64), `role` VARCHAR(20), `wear_site` VARCHAR(30) NULL, `assigned_at_ms`, `unassigned_at_ms` NULL. 인덱스 `(device_id, unassigned_at_ms)`.
...
- **열기**: 회원 존재 → 회원의 OPEN 회차 없음 → 각 기기의 다른 OPEN 회차 미배정 → retention 검증 → `run_id` 발급 → 저장(회차+배정 같은 트랜잭션).
- **닫기**: OPEN만. `ended_at_ms`(기본 now, `≥ started`) 기록, 미해제 배정은 같은 시각으로 해제, `status=CLOSED`.
- **배정/해제**: OPEN 회차만. 해제 시각 `≥ assigned`.
...
- **배치 귀속** (`SensorBatchService.ingest`, 검증 뒤·S3 앞): `request.run_id != null`이면 그대로. null이면 `is_resend && original_run_id != null`이면 그것. 아니면 회원의 OPEN 회차 중 그 `device_id`가 `measured_at_start_ms` 시점에 배정된 것(`assigned_at ≤ start < unassigned_at or null`)을 찾아 `run_id`·`study_id`·`participation_id`를 채운다. 없으면 null 유지(운영 외 자료). 조회는 `CollectionRunService.resolveRun(memberId, deviceId, measuredAtStartMs)`.
...
| 코드 | HTTP | 조건 |
| --- | --- | --- |
| `RUN_4041 RUN_NOT_FOUND` | 404 | `runId`·`assignmentId` 없음 |
...
- [x] 회원에게 OPEN 회차가 있으면 새 회차 열기는 409, 기기가 다른 OPEN 회차에 배정돼 있으면 409.
...
- [x] `run_id` null인 배치가 열린 회차·배정 기기에 맞으면 `run_id`·`study_id`·`participation_id`가 채워지고, 재전송은 `original_run_id`를 우선한다.
