...
구버전 access/refresh(버전 claim 누락)와 ws-token(회원 ID만 저장)은 거절한다. 배포 후 기존 클라이언트는 재로그인해야 한다. 2026-09-14 사용자가 재로그인 적용과 구 서버 혼합 운영 금지 조건을 승인했다. 버전 0으로 간주하는 호환은 폐기 우회 위험 때문에 제공하지 않는다. 만료 access만으로 재발급하는 메서드는 재발급하지 않는다.
...
- [x] 구버전 토큰 거절·배포 후 재로그인·구 서버 혼합 운영 금지: 2026-09-14 사용자 승인.
- [ ] 재로그인 안내 문구·전환 시각·구 서버 중지 실행 담당자는 운영에서 확인한다. 정책 승인은 실제 배포 실행 승인이 아니다.
