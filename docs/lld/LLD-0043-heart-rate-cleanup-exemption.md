...
환경변수는 쉼표 구분 회원 ID 목록이다(예: `1023,1077`). 비어 있으면 제외 없음.
...
신규 에러 코드 없음. 설정값이 숫자가 아니면 기동 시 바인딩 실패로 뜨지 않는다. 잘못된 값으로 삭제가 도는 것보다 낫다.
...
DB 마이그레이션 없음. 실증 서버 배포 설정에 `HEART_CLEANUP_EXEMPT_MEMBER_IDS`를 추가한다. 값이 없으면 지금과 같다.
