...
3. `runId`가 null이면 연구 판정 행을 만들지 않는다. 회차에 귀속된 배치에 한해, 루프 뒤 **중복 skip을 뺀 저장 샘플이 1개 이상**이고 `aiTargetCount == 0`이고 AI가 한 번도 호출되지 않았다면 → `ABSTAIN_INSUFFICIENT_INPUT` 행 1개(배치 단위). AI 호출 실패로 대상이 0이 된 경우는 제외한다. **중복 skip된 샘플은 세지 않는다** — 이미 판정한 시각이 다시 온 것이라 입력이 부족한 것이 아니다. 그래서 전부 중복인 재전송 배치는 아무 행도 남기지 않는다(남기면 재전송 횟수만큼 없던 「판정 불가」가 쌓여 사후 분석이 오염된다).
...
- [ ] 배치 경로라도 `run_id` 귀속에 실패하면 `ALERT`·`ABSTAIN` 연구 판정 행을 만들지 않고 기존 심박 저장·보호자 알림을 유지한다.
