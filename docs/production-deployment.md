...
3. /home/ec2-user/.env를 운영 값으로 준비한다. 공통 docker-compose.yml의 환경변수를 기준으로 RDS_ENDPOINT, RDS_PORT, RDS_USERNAME, RDS_PASSWORD, DB_NAME, REDIS_PASSWORD를 지정한다. JWT 변수는 JWT_ACCESS_TOKEN_SECRET, JWT_REFRESH_TOKEN_SECRET, JWT_TEMPORARY_TOKEN_SECRET과 각 EXPIRATION_TIME 이름을 사용한다. FCM outbox 배포 시 기존 `FCM_DELIVERY_MAX_RETRIES`, `FCM_DELIVERY_NORMAL_TTL`, `FCM_DELIVERY_EMERGENCY_TTL` 값이 승인 기본값(5, 86400s, 300s)과 일치하는지 확인한다. 파일을 source하거나 내용을 로그에 출력하지 않는다.
...
6. RDS 스냅샷과 복구 절차를 확보한다. 빈 DB는 전체 기준 스키마를 먼저 준비한다. 기존 DB는 결제 멱등 키·PG 트랜잭션 분리·포인트 version·FULLTEXT 인덱스 등의 미적용 변경을 확인해 사전 적용한다. FCM outbox 배포 전에는 [create_fcm_outbox.sql](../scripts/mysql/create_fcm_outbox.sql)을 운영과 동등한 MySQL에 적용하고 생성된 테이블·FK·인덱스를 확인한다. 이 SQL은 기존 legacy 알림을 수정하지 않는다. validate는 스키마를 생성하지 않는다.
