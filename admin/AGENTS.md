...
`admin/` 디렉터리는 React + TypeScript 기반 운영 모니터링 대시보드(SPA)입니다. Spring Boot 백엔드와 별개로 동작합니다. 공통 규칙은 루트 [AGENTS.md](../AGENTS.md)를 참조하세요.
...
`package-lock.json`을 기준으로 `npm ci`를 사용한다. `npm run lint`와 `npm run build`를 실행한다. Node 버전은 `package.json`의 도구 요구사항 및 CI와 맞춘다. 화면 동작 변경은 가능하면 브라우저에서 해당 흐름도 확인하고, 미확인 사항을 보고한다.
