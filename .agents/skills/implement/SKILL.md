...
- LLD에서 번호와 관계없이 `미결정 사항` 또는 `Open Questions` 제목의 섹션을 찾는다. 해당 섹션이 비어 있지 않으면 사용자에게 확인하고 진행한다.
- 변경 파일마다 적용되는 `AGENTS.md`를 저장소 루트부터 파일이 있는 디렉터리까지 순서대로 읽는다. 해당 영역의 `CLAUDE.md`도 함께 읽어 기존 호환 규칙을 따른다.
...
3. LLD에서 번호와 관계없이 `미결정 사항` 또는 `Open Questions` 제목의 섹션을 확인하고, 미결정 항목이 있으면 사용자에게 알린다.
